...
loop_sin:
    ...
    blt loop_sin        ; Voltar ao loop se n < terms
...
/* Sobre a função sin_asm
    ...
    mov r1, #0: Inicializa o registrador r1 com 0.0 para armazenar o resultado.

    mov r2, #1: Inicializa o registrador r2 com 1 para manter o controle do sinal dos termos da série.

    mov r3, #0: Inicializa o registrador r3 com 0 para contar os termos da série.

    loop_sin:: Marcador para o início do loop.

    ldr r5, =2: Carrega o valor 2 para o registrador r5.

    mul r6, r5, r3: Multiplica o valor de r5 (2) pelo valor de r3 (contador) e armazena o resultado em r6. Isso calcula 2 * n.

    add r5, r6, #1: Adiciona 1 ao resultado anterior (2 * n) e armazena o resultado em r5. Isso calcula 2 * n + 1.

    bl pow_asm: Chama a função pow_asm para calcular pow(x, 2 * n + 1).

    ldr r6, =2: Carrega o valor 2 para o registrador r6.

    mul r6, r6, r3: Multiplica o valor de r6 (2) pelo valor de r3 (contador) e armazena o resultado em r6. Isso calcula 2 * n.

    add r6, r6, r2, lsl #31: Adiciona o valor de r2 (sinal) deslocado 31 bits para a esquerda ao resultado anterior (2 * n) e armazena o resultado em r6. Isso calcula sign * x^(2 * n).

    bl fat_asm: Chama a função fat_asm para calcular fat(2 * n + 1).

    mul r0, r6, r0: Multiplica o valor de r6 (sign * x^(2 * n)) pelo valor de r0 (pow(x, 2 * n + 1)) e armazena o resultado em r0. Isso calcula (sign * x^(2 * n)) / fat(2 * n + 1).
    ...
    blt loop_sin: Se o valor de r3 (contador) for menor que o valor de r4 (número de termos), o fluxo de controle retorna ao marcador loop_sin, continuando o loop.
